...
export async function paymentsRoutes(app: FastifyInstance) {
  ...
  app.register(async (protectedApp) => {
    ...
    protectedApp.withTypeProvider<ZodTypeProvider>().post('/pix', {
      schema: { tags: ['Pagamentos'], summary: 'Gera QR Code Pix', body: createPixSchema }
    }, controller.generatePix.bind(controller));

    protectedApp.withTypeProvider<ZodTypeProvider>().post('/link', {
      schema: { tags: ['Pagamentos'], summary: 'Gera link de checkout (Cartão/Boleto)', body: createLinkSchema }
    }, controller.generateLink.bind(controller));

    protectedApp.withTypeProvider<ZodTypeProvider>().get('/:id/status', {
      schema: { tags: ['Pagamentos'], summary: 'Verifica o status atual do pagamento', params: idParamSchema }
