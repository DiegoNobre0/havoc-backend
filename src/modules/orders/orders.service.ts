...
export class OrderService {
    ...
    // 3. Atualiza o status e gera o log automático no histórico
    ...
    async updateStatus(id: string, status: any, note?: string) {
        ...
        try {
            if (order.customerPhone) {
                ...
                const WhatsAppService = (await import('../../integrations/whatsapp/whatsappIntegration.service.js')).WhatsAppIntegrationService;
                ...
                if (mensagemAutomativa) {
                    ...
                    console.log(`[WhatsApp] 📲 Notificação de Rastreio (${status}) enviada para ${order.customerPhone}`);
                    
                    // Salva essa mensagem do "Sistema" no histórico do chat para o atendente ver que foi enviado
                    const session = await prisma.chatSession.findUnique({ where: { sessionKey: order.customerPhone } });
                    if (session) {
                        await prisma.chatMessage.create({
                            data: { sessionId: session.id, role: 'ASSISTANT', content: `[Rastreio Automático] ${mensagemAutomativa}` }
                        });
                    }
                }
            ...
            console.error('[Sync Error] ⚠️ Erro ao sincronizar notificações do Kanban com WhatsApp:', error);
        }
